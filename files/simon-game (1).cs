...
@code {
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            // Load the sound service JavaScript
            await JSRuntime.InvokeVoidAsync("eval", "if (!window.soundService) { var script = document.createElement('script'); script.src = 'js/sound-service.js'; document.head.appendChild(script); }");
        }
    }
    ...
    private async Task PlayTileSound(int tileIndex)
    {
        // Play sound using JS interop
        await JSRuntime.InvokeVoidAsync("soundService.playTone", tileIndex);
    }
    
    private async Task PlayErrorSound()
    {
        // Play error sound using JS interop
        await JSRuntime.InvokeVoidAsync("soundService.playError");
    }
